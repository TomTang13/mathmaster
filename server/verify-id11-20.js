...
async function verify() {
  ...
  try {
    ...
    const [totalRows] = await connection.execute(
      'SELECT COUNT(*) as total FROM question_base WHERE knowledge_id BETWEEN 11 AND 20'
    );
    console.log('=== 验证结果 ===');
    console.log(`总题目数: ${totalRows[0].total} (期望: 400)`);
    ...
    const [typeStats] = await connection.execute(
      'SELECT knowledge_id, question_type_id, COUNT(*) as count FROM question_base WHERE knowledge_id BETWEEN 11 AND 20 GROUP BY knowledge_id, question_type_id ORDER BY knowledge_id, question_type_id'
    );
    ...
    const [diffStats] = await connection.execute(
      'SELECT knowledge_id, difficulty_id, COUNT(*) as count FROM question_base WHERE knowledge_id BETWEEN 11 AND 20 GROUP BY knowledge_id, difficulty_id ORDER BY knowledge_id, difficulty_id'
    );
    console.log('\n按难度统计:');
    ...
    const [fillBlankCheck] = await connection.execute(
      `SELECT question_id, question_body, answer 
       FROM question_base 
       WHERE knowledge_id BETWEEN 11 AND 20 AND question_type_id = 1 
    ...
    const [choiceCheck] = await connection.execute(
      `SELECT question_id, question_body, answer 
       FROM question_base 
       WHERE knowledge_id BETWEEN 11 AND 20 AND question_type_id = 2 
    ...
    const [tfCheck] = await connection.execute(
      `SELECT question_id, question_body, answer 
       FROM question_base 
       WHERE knowledge_id BETWEEN 11 AND 20 AND question_type_id = 3 
