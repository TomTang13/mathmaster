...
async function verifyQuestionBase() {
  ...
  try {
    console.log('========== question_base 验证报告 (id 3,4,5) ==========\n');

    // 1. 总数
    const [total] = await connection.execute(
      'SELECT COUNT(*) as count FROM question_base WHERE knowledge_id IN (3,4,5)'
    );
    ...
    const [byKnowledge] = await connection.execute(
      `SELECT knowledge_id, COUNT(*) as total,
       ...
       FROM question_base WHERE knowledge_id IN (3,4,5) GROUP BY knowledge_id ORDER BY knowledge_id`
    );
    ...
    const [choices] = await connection.execute(
      `SELECT question_id, answer, question_body FROM question_base 
       WHERE knowledge_id IN (3,4,5) AND question_type_id = 2 ORDER BY question_id LIMIT 10`
    );
    let validAnswers = 0;
    ...
    const [tfs] = await connection.execute(
      `SELECT question_id, answer, question_body FROM question_base 
       WHERE knowledge_id IN (3,4,5) AND question_type_id = 3 ORDER BY question_id`
    );
    let validTf = 0;
    for (const q of tfs) {
    ...
    const [fills] = await connection.execute(
      `SELECT question_id, answer, question_body FROM question_base 
       WHERE knowledge_id IN (3,4,5) AND question_type_id = 1 ORDER BY question_id`
    );
    let validFill = 0;
    ...
    const [samples] = await connection.execute(
      `SELECT question_id, question_type_id, difficulty_id, question_body, answer, options_json 
       FROM question_base WHERE knowledge_id IN (3,4,5) ORDER BY knowledge_id, question_type_id, difficulty_id LIMIT 9`
    );
    ...
    console.log(`✓ 知识点3: 20题 (12单选+3判断+5填空)`);
    console.log(`✓ 知识点4: 20题 (12单选+3判断+5填空)`);
    console.log(`✓ 知识点5: 20题 (12单选+3判断+5填空)`);
